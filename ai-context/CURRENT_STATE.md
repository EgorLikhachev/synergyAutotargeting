...
## Следующие шаги
1. **Борд-валидация проходки** (как борд включат): deploy.sh -> bench
   до/после (`tools/bench.sh --full` теперь с реальным детектором и diag-
   телеметрией; было: bench без [logging] читал пустоту) +
   `cargo test -p capture --release grbg_demosaic_bench -- --ignored` на
   A76 -> решение по NEON. Replay-метрики не хуже + экспресс-soak 1-2 ч.
   Затем тег 0.2.0.
2. Ретест UI заказчиком (Этап 3, ~30 мин): АРМ/fail-safe/unlock/запись +
...
3. Сбор данных R1 (заказчик, docs/data_collection.md): записи в
   dataset/raw/ + `python tools/dataset_check.py dataset/raw`.
4. Полевые — по чеклисту wiring_gep_f405.md §9 (после ретеста); следить
   за температурой: 80°C уже при стендовом трекинге.
5. R1-трейн — когда наберётся 15-20 роликов (метрики до/после —
