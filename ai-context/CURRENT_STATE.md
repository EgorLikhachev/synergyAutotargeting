...
- **Аудит-проходка 2026-09-10** (c224a80..801fc49, борд выключен): ранний
  отсев YOLO-декодера decode_floor (бит-идентичен, ~-0.5M exp/детекцию),
  Cow-Img — копий кадра 0.9 МБ за кадр было 3-5, осталась 1 (запрос
  детектору), zune-jpeg вместо jpeg-decoder (пульт/replay/MJPEG, ~2-4x),
  событийный repaint пульта, построчный демозаик GRBG (бит-паритет,
  x86 лишь ~7% — см. грабли/бенчмарк), write-timeout'ы fail-safe
  (дыра dead-man в UI-CTRL закрыта), статус 15 Гц. ЗАМЕР НА БОРДУ
  ПЕНДИНГ (bench --full + grbg_demosaic_bench): все цифры «Что работает»
  ниже — до борд-валидации проходки.
